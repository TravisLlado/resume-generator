...
- **`init prompt.md`** — one-time setup. Creates `stories/` (remote-backed or local-only, your choice) so the other two prompts have something to read from and write to, and adds a local git safety net so your data can never leak to this repo's public origin.
